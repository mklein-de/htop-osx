...
char *Process_fieldNames[] = {
  ...
  "WCHAN",
  "NSWAP", "CNSWAP", "EXIT_SIGNAL", "PROCESSOR", "M_SIZE", "M_RESIDENT",
  "M_SHARE",
  "M_TRS", "M_DRS", "M_LRS", "M_DT", "ST_UID", "PERCENT_CPU",
  "PERCENT_MEM",
  "USER", "TIME", "NLWP", "TGID",
#ifdef HAVE_OPENVZ
  "CTID", "VPID",
...
static void
Process_printLargeNumber( Process * this, RichString * str,
  ...
  if ( number >= ( 10 * ONE_M ) ) {
    ...
    int attr = this->pl->highlightMegabytes ? CRT_colors[PROCESS_MEGABYTES]
        : CRT_colors[PROCESS];
...
static void
Process_printTime( RichString * str, unsigned long t ) {
  if ( jiffy == 0.0 )
...
static inline void
Process_outputRate( Process * this, RichString * str, int attr,
                    char *buffer, int n, double rate ) {
  rate = rate / 1024;
  if ( rate < 0.01 )
    snprintf( buffer, n, "    0 " );
  else if ( rate <= 10 )
...
static void
Process_writeField( Process * this, RichString * str, ProcessField field ) {
  ...
  switch ( field ) {
  case PID:
    snprintf( buffer, n, "%5u ", this->pid );
    break;
  case PPID:
    snprintf( buffer, n, "%5u ", this->ppid );
    break;
  case PGRP:
    snprintf( buffer, n, "%5u ", this->pgrp );
    break;
  case SESSION:
    snprintf( buffer, n, "%5u ", this->session );
    break;
  case TTY_NR:
    snprintf( buffer, n, "%5u ", this->tty_nr );
    break;
  case TGID:
    snprintf( buffer, n, "%5u ", this->tgid );
    break;
  case TPGID:
    snprintf( buffer, n, "%5d ", this->tpgid );
    break;
  case PROCESSOR:
  ...
  case COMM:{
      if ( this->pl->highlightThreads
           && ( this->pid != this->tgid || this->m_size == 0 ) ) {
  ...
  case STATE:{
      ...
      attr = this->state == 'R' ? CRT_colors[PROCESS_R_STATE]
          : attr;
      break;
    }
  case PRIORITY:{
  ...
  case M_DRS:
    Process_printLargeNumber( this, str, this->m_drs * PAGE_SIZE_KB );
    return;
  case M_DT:
    Process_printLargeNumber( this, str, this->m_dt * PAGE_SIZE_KB );
    return;
  case M_LRS:
    Process_printLargeNumber( this, str, this->m_lrs * PAGE_SIZE_KB );
    return;
  case M_TRS:
    Process_printLargeNumber( this, str, this->m_trs * PAGE_SIZE_KB );
    return;
  case M_SIZE:
    Process_printLargeNumber( this, str, this->m_size * PAGE_SIZE_KB );
    return;
  case M_RESIDENT:
    Process_printLargeNumber( this, str, this->m_resident * PAGE_SIZE_KB );
    return;
  case M_SHARE:
    Process_printLargeNumber( this, str, this->m_share * PAGE_SIZE_KB );
    return;
  case ST_UID:
...
#ifdef HAVE_OPENVZ
  case CTID:
    snprintf( buffer, n, "%5u ", this->ctid );
    break;
  case VPID:
    snprintf( buffer, n, "%5u ", this->vpid );
    break;
#endif
#ifdef HAVE_VSERVER
  case VXID:
...
#ifdef HAVE_TASKSTATS
  case RCHAR:
    snprintf( buffer, n, "%10llu ", this->io_rchar );
    break;
  case WCHAR:
    snprintf( buffer, n, "%10llu ", this->io_wchar );
    break;
  case SYSCR:
    snprintf( buffer, n, "%10llu ", this->io_syscr );
    break;
  case SYSCW:
    snprintf( buffer, n, "%10llu ", this->io_syscw );
    break;
  case RBYTES:
    snprintf( buffer, n, "%10llu ", this->io_read_bytes );
    break;
  case WBYTES:
    snprintf( buffer, n, "%10llu ", this->io_write_bytes );
    break;
  case CNCLWB:
    snprintf( buffer, n, "%10llu ", this->io_cancelled_write_bytes );
    break;
  case IO_READ_RATE:
    Process_outputRate( this, str, attr, buffer, n,
                        this->io_rate_read_bps );
    return;
  case IO_WRITE_RATE:
    Process_outputRate( this, str, attr, buffer, n,
                        this->io_rate_write_bps );
    return;
  case IO_RATE:
    Process_outputRate( this, str, attr, buffer, n,
                        this->io_rate_read_bps + this->io_rate_write_bps );
    return;
#endif
...
static void
Process_display( Object * cast, RichString * out ) {
...
Process *
Process_new( struct ProcessList_ *pl ) {
...
unsigned long
Process_getAffinity( Process * this ) {
  return 0;
}
...
int
Process_pidCompare( const void *v1, const void *v2 ) {
...
int
Process_compare( const void *v1, const void *v2 ) {
  Process *p1, *p2;
  ...
  case IO_RATE:
    diff =
        ( p2->io_rate_read_bps + p2->io_rate_write_bps ) -
        ( p1->io_rate_read_bps + p1->io_rate_write_bps );
    goto test_diff;
#endif
